...
class TftpPacketTest {
    ...
    void testGoodACK() {
        ...
        DatagramPacket dp = TftpPacket.createACK(ia, 69, 190);
        assertNotNull(dp);
        TftpPacket tp = TftpPacket.parse(dp);
        assertNotNull(tp);
    ...
    void testGoodERR() {
        ...
        assertEquals("error message", tp.getError(), "error is 'error message'");
    }
    ...
    void testNextBlock() {
        for (int i = 1; i < 255; i++) {
            assertEquals(i + 1, TftpPacket.nextBlock(i), "next block is " + (i + 1) + "for current = " + i);
        }

        assertEquals(1, TftpPacket.nextBlock(255), "next block is 1 for current = 255");
    }
    ...
    void testLastBlock() {
        for (int i = 2; i <= 255; i++) {
            assertEquals(i - 1, TftpPacket.lastBlock(i), "last block is " + (i - 1) + "for current = " + i);
        }

        assertEquals(255, TftpPacket.lastBlock(1), "last block is 255 for current = 1");
    }
